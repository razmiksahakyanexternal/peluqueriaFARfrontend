...
export class RegistroExitosoComponent implements OnInit {
  message = 'Hemos enviado un correo de verificación a tu email.';
  ...
  resendVerificationEmail(): void {
    ...
    this.authService.resendVerificationEmail(this.email).subscribe({
      next: (response) => {
        ...
        this.resendMessage = response?.message ?? 'Si la cuenta existe, se ha enviado un correo de verificación.';
      },
      error: (error) => {
        this.resendLoading = false;
        this.resendError =
